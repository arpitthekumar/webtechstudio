...
const BlogSection = ({ section }) => (
  <div className="space-y-5 px-4 md:px-0">
    ...
    {section.keypoints?.map((kp, i) => (
      <div key={i} className="p-2">
        ...
        <p className="text-lg text-bluish-gray pt-2">
          {kp.explanation} {kp.link && <Link href={kp.link} className="text-acua-marine hover:underline">Read more</Link>}
        </p>
      </div>
    ))}
    ...
    {section.conclusion && <p className="text-lg text-bluish-gray italic pl-4">{section.conclusion}</p>}
  </div>
);
